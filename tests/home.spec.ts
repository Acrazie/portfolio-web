...
test('SSR and lazy shader graph enhance without runtime errors',async({page})=>{
 ...
 await page.goto('/')
 await expect(page.getByRole('heading',{level:1})).toContainText('Software. With a new perspective.')
 await expect(page.getByTestId('execution-graph-fallback')).toBeAttached()
