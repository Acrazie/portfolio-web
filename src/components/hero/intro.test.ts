...
it('sweeps distinct spiral arms through depth before decelerating precisely onto original targets',()=>{
 const pose=modules['./intro.ts'].introPosition;expect(pose).toBeTypeOf('function')
 const t={x:80,y:40},f={x:130,y:-90,z:-200}
 const a=pose(t,f,0,1),b=pose(t,f,.3,1),otherArm=pose(t,f,.3,3)
 expect(Math.hypot(b.x-a.x,b.y-a.y)).toBeGreaterThan(80)
 expect(Math.hypot(otherArm.x-b.x,otherArm.y-b.y)).toBeGreaterThan(80)
