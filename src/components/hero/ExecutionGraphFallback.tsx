...
export function ExecutionGraphFallback() {
 const {nodes,edges}=createGraphFrame({width:800,height:560,progress:0,seed:42})
 return <div className="absolute inset-0" data-testid="execution-graph-fallback">
  ...
  <ol aria-label="Execution flow" className="absolute inset-0">
   {nodes.map(n=><li key={n.id} style={{left:`${n.x/8}%`,top:`${n.y/5.6}%`}} className={`absolute z-10 -translate-x-1/2 -translate-y-1/2 border px-2 py-2 font-mono text-[9px] uppercase tracking-wider sm:px-3 sm:text-[11px] ${n.label==='AC'?'border-[#b4b6ff] bg-[#b4b6ff] px-4 py-4 text-[#191a28]':'border-white/20 bg-[#191a24] text-[#dddde8]'}`}>{n.label}</li>)}
  </ol>
 </div>
}
