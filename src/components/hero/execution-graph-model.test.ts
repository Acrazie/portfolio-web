...
it('samples source ink while mapping its tones into the optical spectrum', () => {
 ...
 for(let i=0;i<p.color.length;i+=3){
  expect(p.color[i]).toBeGreaterThanOrEqual(.25)
  expect(p.color[i]).toBeLessThanOrEqual(.55)
  expect(p.color[i+1]).toBeGreaterThanOrEqual(.32)
  expect(p.color[i+1]).toBeLessThanOrEqual(.54)
  expect(p.color[i+2]).toBeGreaterThanOrEqual(.78)
  expect(p.color[i+2]).toBeLessThanOrEqual(.97)
 }
})
