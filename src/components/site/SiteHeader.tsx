...
export function SiteHeader({ active, home = false }: { active: NavKey; home?: boolean }) {
  ...
  return (
    <header className={`z-40 text-white ${home ? 'absolute inset-x-0 top-0' : 'relative bg-black'}`}>
      <div className="site-container flex min-h-20 items-center justify-between gap-6">
        {home ? <span aria-hidden="true" /> : (
          <Link to="/" className="focus-ring text-sm font-semibold tracking-[-.02em] text-white">
            {portfolio.displayName.toUpperCase()}
          </Link>
        )}

        <div className="flex items-center gap-2">
          <nav aria-label={copy.navigationLabel} className="hidden items-center gap-6 md:flex">
            {navItems.map(item => (
              <Link
                key={item.key}
                to={item.to}
            ...
            <a className="focus-ring inline-flex min-h-11 items-center text-sm text-white/78 hover:text-white" href={`mailto:${portfolio.email}`}>
              {copy.nav.contact}
            </a>
          </nav>
          <button
          ...
          >
            {copy.language.short}<span aria-hidden="true" className="px-1 text-white/35">/</span><span aria-hidden="true" className="text-white/55">{copy.language.other}</span>
          </button>
          <button
            type="button"
      ...
      {open && (
        <div className="fixed inset-0 z-50 bg-black text-white md:hidden">
          ...
          <nav aria-label={copy.navigationLabel} className="site-container flex flex-col py-10">
            {navItems.map(item => (
              <Link
                key={item.key}
                to={item.to}
            ...
            <a className="focus-ring border-b border-white/18 py-5 text-3xl font-medium tracking-[-.03em]" href={`mailto:${portfolio.email}`}>
              {copy.nav.contact}
            </a>
          </nav>
        </div>
